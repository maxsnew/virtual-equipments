...
\begin{figure}
  \begin{mathpar}
    ...
    \id_h \circ_{vs} \gamma = \id_h

    (h \circ \id) = h\and \id \circ h = h

    % theory of an indexed category (horizontal)

    % embedding of displays

    % strict equipment (horizontal)
    % strict equipment  (displays)
    ...
    % 2-cells: transformations : Q: is any of this necessary or should
    % we just conflate transformations with substitutions into a
    % display?
    % 
    % action of trans on subst with assoc, unit
    % identity trans with unit
    % embedding of transformations into substitutions
    ...
    \inferrule
    {\psi : \algHSubst(f',g',\Phi,\Psi)\and \phi : \algHSubst(f,g,\Sigma,\Phi)
    }
    {\psi \circ \phi : \algHSubst(f' \circ f, g' \circ g, \Sigma, \Psi)}

    \inferrule
    {}
    {restrict(f,g,\phi) : \algHSubst(\id,\id,\Psi,\Phi)}
  \end{mathpar}
  \caption{Equipment with Displays}
