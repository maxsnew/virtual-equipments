...
To simplify our initiality proof, we use an \emph{essentially
algebraic} presentation of our notion of model. To describe this, we
give a ``combinator'' syntax for VE hyperdoctrines, where we have
explicit sorts for contexts $\Gamma$ and $\Phi$ and explicit judgments
for substitutions $\gamma : \Delta \to \Gamma$ and $\phi : \Psi
\vdash_{f,g} \Phi$.
%
...
\begin{figure}
  \begin{mathpar}
    ...
    \inferrule
    {\Delta : \isaCtx \and \Gamma : \isaCtx}
    {\gamma : \textrm{Subst}(\Delta,\Gamma)}
    ...
    \inferrule
    {\Gamma : \isaCtx \and C : \textrm{Cat}(\Gamma) \and D : \textrm{Cat}(\Gamma) \and
    }
    {F : \textrm{Functor}(\Gamma,C,D)}
    ...
    \inferrule
    {\Gamma : \isaCtx \and C : \textrm{Cat}(\Gamma) \and D : \textrm{Cat}(\Gamma)
    }
    {P : \textrm{ProfCtx}(\Gamma,C,D)}

    \inferrule
    {\Gamma : \textrm{Ctx}\and
      C  : \textrm{Cat}(\Gamma) \and D  : \textrm{Cat}(\Gamma) \and
      C' : \textrm{Cat}(\Gamma) \and D' : \textrm{Cat}(\Gamma) \and
      f : \textrm{Functor}(\Gamma,C,C') \and g : \textrm{Cat}(\Gamma,D,D') \and
...
\begin{figure}
  \begin{mathpar}
    \inferrule
    {g : \textrm{Functor}(D,E) \and \textrm{Functor}(C,D)}
    {g \circ_{\Gamma,C,D,E} f : \textrm{Functor}(C,E)}

    (h \circ g) \circ f = h \circ (g \circ f)

    \inferrule
    {}
    {\id_C : \textrm{Functor}(C,C)}
    ...
    \inferrule
    {P : \textrm{Profunctor}(\Gamma,C',D')
    \and f : \textrm{Functor}(\Gamma,C,C')\and
    \and g : \textrm{Functor}(\Gamma,D,D')}
    {P \circ_{\Gamma,C,D,C',D'} (f,g) : \textrm{Profunctor}(\Gamma,C,D)}

    P \circ (f,g) \circ (f',g') = P \circ (f \circ f', g \circ g')

    P \circ (\id,\id) = P

    \inferrule
    {\psi : \textrm{ProfSubst}(f',g',\Phi,\Psi)\and \phi : \textrm{ProfSubst}(f,g,\Sigma,\Phi)
    }
    {\psi \circ \phi : \textrm{ProfSubst}(f' \circ f, g' \circ g, \Sigma, \Psi)}

    \inferrule
    {}
    {restrict(f,g,\phi) : \textrm{ProfSubst}(\id,\id,\Psi,\Phi)}
  \end{mathpar}
  \caption{Algebraic Presentation of SCT}
\end{figure}
