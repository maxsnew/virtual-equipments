...
languages can be soundly and completely modeled
by notions of structured categories\cite{lawvere,goguen,lambek,moggi}, a
...
simple proofs in category theory. For instance consider the proof of the
Yoneda lemma, often the first ``big'' proof in an introductory category
theory text. The Yoneda lemma states that for any category $C$, functor
$P : C^{o} \to \Set$ (where $C^{o}$ is the ``opposite'' of $C$, i.e. 
$C$ with the direction of morphisms reversed) and object $A \in C$, the
set of natural transformations $C(-,A) \to P$ is in bijection with the
elements $P(A)$. Furthermore, this bijection is natural in both $A$ and
$P$. In the authors' experience teaching category theory the sheer
amount of naturality side conditions in this result can be very
intimidating to learners, while experienced practitioners will observe
that these naturality conditions are tedious to carry out but
routine. In the type theory we introduce in this paper, none of these
naturality conditions need to be explicitly proven. Instead, all terms
expressible in the type theory are automatically natural by
construction.
...
$\epsilon\mathord{-}\delta$ definition of continuity and prove that it is
...
\vett{} is an \emph{ordered}, \emph{constructive} variant of
...
and so we leave these extensions to future work.
%
...
categories inside the ordered part of the type theory, we can state a
theorem that quantifies over functors using the meta-language's
universal quantifier.  To more easily formalize these metatheorems, we
add a third layer to the type theory: an ordinary dependent type theory
in the style of Martin-L\"of type theory. Then all of our ordered
predicate logic judgments will also be indexed by a context from this
dependent type theory, and the type theory includes types for
categories, functors, profunctors and natural transformations that allow
us to formalize theorems the logic is too restrictive to encode. This is
analogous to the notion of a \emph{2-level type
  theory}\cite{voevodsky12,altenkirchXX}: we add a dependent type theory
for reasoning about the terms of our virtual equipment type theory.
...
variables.  A typical $\Phi$ will look like
\[
\alpha_1 : \cat C_1, x_1:R_1(\alpha_1,\alpha_2), \alpha_2 : \cat C_2,
x_2:R_2(\alpha_2,\alpha_3), \alpha_3:\cat C_3, \ldots
\]
...
The \emph{formation conditions} or \emph{presuppositions} of a judgement
are the conditions under which a judgement ``type checks''.  We have
\begin{itemize}
\item ${\Gamma\vdash \cat C \isaCat}$ presupposes ${\Gamma \isadtctx}$
\item ${\Gamma \pipe \alpha : \cat C \vdash a : \cat D}$ presupposes
       $\Gamma \vdash \cat C\isaCat$ and $\Gamma \vdash \cat D \isaCat$.
\item ${\Gamma \pipe \Xi \vdash S \isaSet}$ presupposes
      $\Gamma \isadtctx$ and $\Gamma \vdash \Xi \boundary$
\item ${\Gamma \pipe \Phi \vdash s : R}$ presupposes
...
All other forms are typed with respect to a well-formed type context
$\Gamma$. Objects are typed additionally with an input object variable
$\alpha : \cat C$ and an output category $\cat D$; in the semantics
these objects are modeled as functors. A set $S$ is typed with respect
...
profunctors. A transformation $s$ has a context of transformation
...
ordinary type theoretic substitution. Objects $\alpha:\cat C
...
Recall that the basic entities of our type theory are categories,
objects/functors, sets/profunctors, and elements/natural
transformations.  In this section we discuss some connectives for
constructing categories, which are specified by introduction and
elimination rules (the $\beta\eta$ equality and substitution rules are
included in the appendix).  The introduction and elimination rules make
use of functors, profunctors, and natural transformations.
%
We introduce the connectives for defining categories in
Figure~\ref{fig:category-connectives}. 
%
...
the sets, i.e., constructions on profunctors.  In these constructions,
the sets/profunctors act like types, and they are given 
introduction and elimination rules for elements/natural
transformations.  
%
...
for these sets (the $\beta/eta$-rules are in the appendix).  
%
...
in the form of a continuation $\alpha. t$. Next, the tensor product of
...
FIXME: Note that in the unit elimination, the continuation is typed with
a context consisting of only a single object $\alpha$, in contrast with
...
$\Phi$ into $\Phi_l \jnctx \Phi_m \jnctx \Phi_r$ where $\Phi_m$ is used
by the scrutinee and $\Phi_l \jnctx \Phi_r$ is used by the
continuation. However, unlike in ordered linear logic, the $\Phi_l
\jnctx \Phi_r$ is not necessarily well formed because there is no reason
in general that $\Phi_m$ has contravariant and covariant dependence on
the same category.
%
...
categories, functors, profunctors and natural transformations.
%
The rule for the types of small categories and (large) categories are very
...
$\beta\eta$ principles, which state that the introduction and
elimination rules are a bijection between judgement forms.  
%
...
\begin{figure}
  \begin{mathpar}
    ...
    FIXME missing intro/elim rules for smallcat and cat
