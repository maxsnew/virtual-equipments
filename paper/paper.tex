...
More experienced practitioners of category theory in programming
language theory find that for modeling computational effects such as
general recursion or indexed-state, or modeling logics for
parametricity, gradual typing, or step-indexed logical relations
involves not just ordinary category theory, but more general notions
such as enriched, internal or multi-categories. However, model
construction theorems and especially soundness and completeness
theorems in these areas will require their own, even more complex
versions of the same common lemmas of category theory, in addition to
the domain-specific results such as existence of fixed points.
...
%
One approach to taming this complexity is to prove these general
...
flavors of multi-categories that model intuitionistic and
substructural type theories, but also a great deal more, including
double categories, metric spaces and topological spaces.
%
However, there are two difficulties with this approach.
%
First, the algebraic arguments can be difficult to understand compared
to the original argument for ordinary category theory, which is an
obstacle to wider adoption of this powerful approach.
%
Second, the algebraic structure of a virtual equipment is quite
unwieldy to manipulate. The abstraction of a generalized natural
transformation is a kind of ``multi-2-cell'' with many inputs and a
single output. In diagrammatic form these are rendered as
...
Next, we develop the basic model theory for SCT. Because SCT includes
not just a syntax for category theory, but is also indexed by an
ordinary context of dependent type theory $\Gamma$, a model of SCT is
not given by a \emph{single} virtual double category, but instead by
an \emph{indexed family} of VDCs, parameterized by an object of a
...
we need a way to talk about ``profunctors of arbitrarily many
arguments''. This can be accomplished using \emph{multicategory}
theory, which models the structure of the context with its structural
rules as a monad of ``contexts and substitutions''. A suitable notion
of monad on a virtual double category has been developed and was in
fact the original motivation for the concept of a virtual double
