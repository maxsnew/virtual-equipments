...
\begin{abstract}
  ...
  We show that the type theory is sound for models in \emph{virtual
  equipments}, a kind of double category which is known to model not
  just ordinary categories but enriched, internal and generalized
  multi-categories as well. This means that while proofs and
  constructions in our theory read like typical category-theoretic
...
%
First, the abstract arguments can be difficult to understand compapred
to the original argument for ordinary category theory.
%
...
the ``graph'' of $R$ is a category $\exists \alpha;\beta. R$. In
ordinary category theory, this is usually defined as the category of
elements of $R$ viewed as a functor $R : \cat C^o \times D \to \Set$.
In detail, its objects are triples $(a:\cat C, r : R, b : \cat D)$ and
morphism from $(a,r,b)$ to $(a', r', b')$ consists of a pair of
morphisms $g : b \to b'$ and a backwards $f : a' \to a$ such that $r'
= g\circ r \circ f$.
%
...
structure as a \emph{VDC-hyperdoctrine}. We then show how to construct
a VDC-hyperdoctrine indexed by the category of sets, which has the
intuitive reading that any of the VDC judgments $\Gamma \vdash J$ is
interpreted as simply a family of $J$-data parameterized by elements
of the set $\Gamma$.
...
First, we recount the definition of a virtual double category.
\begin{definition}[Virtual Double Category]
  A (vertically strict) virtual equipment $\mathcal V$ consists of
  \begin{enumerate}
    \item A set $\mathcal V_0$ of objects.
    ...
    \item For any vertical arrows $f \in V_v(\cat C, \cat D)$ and $g
      \in V_v(\cat E, \cat F)$ and horizontal arrow $R \in V_h$ with
      $d^+R = \cat F$ and $d^-R = \cat D$, a \emph{restriction}
      $R(f,g)$ with $d^+R = \cat C$ and $d^-R = \cat E$.

      This operation is furthermore associative and unital $R(f \circ
      f',g \circ g') = R(f,g)(f',g')$ and $R(\id,\id) = R$.
      
    \item For each composable string $\Phi$ (defined below) of horizontal arrows
      starting at $\cat C$ and ending at $\cat D$, and horizontal
      arrow $R$ with $d^-R = \cat C$ and $d^+R = \cat D$, a set
      $V_2(\Phi; R)$ of 2-cells.

    \item For any string of vertical arrows $\vec f$ into $\Phi$ and
      2-cell $r \in \mathcal V_2(\Phi,S)$, a restriction operation
      $r(\vec f) \in \mathcal V_2(\Phi(\vec f), S(d^-(\vec f),
      d^+(\vec f)))$. Furthermore, this is associative and unital
      $r(\vec id) = r$ and $r(\vec f \circ \vec g) = r(\vec {f \circ
        g})$
  ...
  A composable string $\Phi$ of horizontal arrows is defined to be an
  alternating sequence of objects and horizontal arrows, beginning and
  ending with an object such that any substring $\cat C,R,\cat D$
  satisfies $d^-R = \cat C$ and $d^+R = \cat D$.

  A composable string of vertical arrows $\vec f$ into $\Phi$ is a
  sequence of vertical arrows, one for each object in $\Phi$, with the
  codomain of the arrow matching the object in $\Phi$. Then 

  A composable string of 2-cells from $\Phi$ to $\Psi$ is a sequence
  $\vec r$ of 2-cells, one for each horizontal arrow in $\Psi$ such
  that the codomains match the objects in
\end{definition}

Our main departure from\cite{crutwellshulman} is making restrictions
part of the definition, and also requiring them to be strictly
associative and unital. In the examples that we are most concerned
with, however, these operations are strict and so we will not explore
the possibility of coherence theorems for proving this equivalent to a
weaker definition.

\begin{definition}
  A functor of virtual double categories $F : \mathcal V \to \mathcal
  U$ is a mapping on objects, vertical arrows, horizontal arrows and
  2-cells respecting typing and preserving composition, units, and
  restriction.
\end{definition}
...
\begin{definition}{Indexed Virtual Equipment}
  A VE-hyperdoctrine is a pair of a category $C$ and a functor $\cat
  C^{op} \to VE$. A morphism of VE-hyperdoctrines
  \[(F,\phi) : (\cat C, \mathcal V : \cat C^{o} \to VE) \to (\cat D, \mathcal U : \cat D^{o} \to VE)\]
  is a pair of a functor $F : \cat C \to \cat D$ and a natural
  transformation $\phi : \mathcal V \Rightarrow\mathcal U \circ F^{o}$.
\end{definition}
...
\begin{definition}{Completeness}
  We define the syntactic VE-hyperdoctrine $\textrm{Syn}$ as follows:
  \begin{enumerate}
  \item The base category is the category of contexts $\Gamma$ and substitutions.
  \item Define $\mathcal V_\Gamma$ as follows
    \begin{enumerate}
    \item An object of $\mathcal V^\Gamma$ is a proof $\Gamma \vdash \cat C \isaCat$.
    \item A vertical morphism $\mathcal V^\Gamma(\cat C, \cat D)$ is a
      term $\Gamma\pipe \alpha:\cat C \vdash t : \cat D$ moduleo
      $\alpha$-equivalence. Composition and identity are given by composition/variable.
    \item A horizontal morphism $R \in \mathcal^\Gamma_h$ is a term
      $\Gamma\pipe \alpha:\cat C;\beta:\cat D \vdash R$ with $d^-R =
      \cat C$ and $d^+R = \cat D$.
    \item Restriction of $R$ along $s,t$ is given by substitution
      $R[s/\alpha;t/\beta]$. This is clearly associative and unital.
    \item A 2-cell from $\Phi$ to $A$ is a term $\Phi \vdash t : B$
    \end{enumerate}
  \end{enumerate}
\end{definition}
...
And we define a profunctor of interpretations of a signature in a
virtual equipment as follows:
\begin{definition}
  An interpretation of a signature $\Gamma$ in a virtual equipment
  $\mathcal V$ is an element of the set $\sem{\Gamma}_{\mathcal
    V}$. Call this set $\textrm{Interp}(\Gamma;\mathcal V)$.

  Note that this constitutes a profunctor contravariant in $\Gamma$
  and covariant in $\mathcal V$.
\end{definition}

Next, we define the syntactic virtual equipment $\textrm{Syn}(\Gamma)$
...
\begin{definition}
  ...
  \item $V^\Gamma_v(\cat C, \cat D)$ is the set of objects in context
    $\Gamma \pipe \alpha:\cat C \vdash a : \cat D$. The category
    structure is given by substitution, with the variable $\alpha
    \vdash \alpha$ the identity.
  \item $V_h$ is the set of triples $(\cat C, \cat D, \Gamma \pipe
    \alpha:\cat C;\beta:\cat D \vdash R \isaSet)$ where $d^\pm$ are
    the projections
...
Then the soundness theorem says that for any $\Gamma$, the virtual
double category $\textrm{Syn}(\Gamma)$ provides an isomorphism of profunctors:
\[ \textrm{Interp}(\Gamma, \mathcal V \cong VE(\textrm{Syn}(\Gamma), \mathcal V) \]
As we saw in a previous section, the backwards direction of this
isomorphism should be a universal interpretation:

\begin{definition}[Universal Interpretation]
  For any signature $\Gamma$ there is a ``self-interpretation'' $\eta:
  \textrm{Interp}(\Gamma, \textrm{Syn}(\Gamma))$ defined as the
  identity substitution $\id \in \sem{\Gamma}_{\textrm{Syn}(\Gamma)}$
...
\begin{definition}[Powers of Virtual Equipments]
  ...
  \item Vertical morphisms $\mathcal V^X_v(A,B)$ are functions
    $\Pi_{x\inX} V_v(A x,B x)$. With pointwise composition, identity.
  ...
  \item A 2-cell from $\Phi$ to $R$ is a family $\Pi_{x\in X} \mathcal
    V^X_h(\Phix,R x)$ with composition given pointwise.
  \end{enumerate}
\end{definition}
...
\begin{definition}[Internal Logic]
  ...
  \item The equipment is defined as $\mathcal U\Gamma = \mathcam V^\Gamma$.
