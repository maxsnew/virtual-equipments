...
\begin{abstract}
  ...
  transformations. By restricting the type theory to an ordered linear
  restriction of predicate logic, we guarantee that all functions
  ...
  separate proof necessary. Important category theoretic proofs such
...
Category theory is a branch of mathematics that gives a systematic
theory of typed, ``higher-dimensional'' algebraic structures.
%
...
that formal systems like logics, type theories and programming
languages can be modeled by sound and complete semantic models given
by notions of structured category\cite{lawvere,goguen,lambek,moggi}, a
phenomenon sometimes referred to as the Curry-Howard-Lambek
nncorrespondence \cite{curry-howard-lambek}.
%
...
and dependent type theory\cite{not-sure}.
%
...
functors and Kan extensions produce a proliferation of ``naturality''
and ``functoriality'' side-conditions that must be discharged.
%
...
step-index-enriched categories have been used to model general
recursion and internal categories have been used to model
...
category theory text. The Yoneda lemma states that for any functor $P
: C^o \to \Set$ and object $A \in C$, that the set of natural
transformations $C(-,A) \to P$ is in bijection with the elements
$P(A)$. Furthermore, this bijection is natural in both $A$ and $P$. In
the authors' experience teaching category theory the sheer amount of
naturality side conditions in this result can be very intimidating to
learners, while experienced practitioners will observe that these
naturality conditions are tedious to carry out but routine. In our
type theory, none of these naturality conditions need to be explicitly
proven. Instead, all terms expressible in the type theory are
automatically natural by construction.
...
$\epsilon-\delta$ definition of continuity and prove that it is
continuous. Rather, they are aware of certain \emph{syntactic
principles}, most importantly that composition of continuous functions
is continuous and that previously defined functions are known to be
continuous.
%
Similar principles apply to category theory itself: a composition of
functors is functorial, a composition of natural transformations is
natural, etc.
%
...
(virtual) double categories and pro-arrow equipments\cite{}.
%
This approach has the advantage of avoiding the need to prove
the side conditions of naturality or enrichment.
%
...
%
In \vett{}, the sorts are interpreted as categories, functions as
functors, relations as profunctors and implications of relations as
profunctor homomorphisms, a generalization of natural transformations.
%
...
category theory, but the proofs are valid in an arbitrary virtual
equipments and so are valid in enriched and internal category theory
as well.
%
...
functions ``for free''.
%
The syntax provides an alternative to algebraic and
string-diagrammatic syntaxes for working with virtual equipments,
...
\vett{} is a kind of \emph{ordered}, \emph{constructive} variant of
predicate logic over a unary type theory.
%
It is \emph{ordered} in that our notion of predicate is that of a
binary relation $R(\alpha;\beta)$ where the $\alpha$ is thought of as
a contravariant dependence and the $\beta$ has a covariant dependence,
and the rules of the logic distinguish between these two variances.
%
Proofs in the predicate logic $R_1, R_2,\ldots \vdash S$ are
restricted as in ordered linear logic \cite{lambek-calculus, ordered-linear-logic} in
that each input must be used exactly once and in the order they were
declared.
%
This order discipline ensures that every proof in the logic can be
interpreted as a \emph{natural} transformation.
%
The logic is \emph{constructive} in that we care about the identity of
proofs: they are natural transformations and there may be more than
one natural transformation of the same type.
%
If we ignore the identity of proofs, ordered predicate logic can be
viewed as a logic of preordered sets rather than categories.
...
categories, opposite categories and arbitrary (co)-ends of
profunctors. However, the restriction is natural in that it
corresponds to a well-understood notion of model (virtual equipments)
and so we leave the extension to future work.
%
It is well known that a great deal of fundamental results and
constructions in category theory such as Yoneda and co-Yoneda lemmas,
adjoint functors and Kan extensions are already expressible in virtual
equipments, for instance virtual equipments are used for this purpose
in a text on $\infty$-category theory to avoid explicit manipulation
of model categories \cite{elements-of-oo-category-theory}.
%
We demonstrate some of these standard constructions
in \ref{sec:synthetic} to show that \vett{} gives a syntax in which
these formal constructions look like standard set-theoretic arguments.

Due to the restriction to unary functors and binary profunctors, it is
difficult to state some theorems internally to the theory. For
instance, since we cannot express functor categories, we cannot
express any theorem that quantifies over functors purely in ordered
predicate logic, and instead only as a metatheorem. To more easily
formalize these metatheorems, we add a third layer to the type theory:
an ordinary dependent type theory in the style of Martin-L\"of type
theory. Then all of our ordered predicate logic judgments will also be
indexed by a context from this dependent type theory, and the type
theory includes types for categories, functors, profunctors and
natural transformations that allow us to formalize theorems the logic
is too restrictive to encode. This is analogous to the notion of a
\emph{2-level type theory}: we add a dependent type theory for
reasoning about the terms of our virtual equipment type theory.

The dependent type theory layer also has an additional purpose: since
we can quantify over the structures in \vett{}, we can additionally
encode \emph{signatures} for \vett{} as simply contexts of the
dependent type theory. This allows for a clean formalization of many
categorical structures such as monoidal categories, or strong monads.
...
\subsection{Basics of Profunctors}
...
introductory texts. First, a category $\cat C$ has a collection of
objects and morphisms with identity and composition, and a functor $F
: \cat C \to \cat D$ is a function on objects and a function on
morphisms that respects source and target. It will be useful for
...
objects satisfying reflexivity and transitivity. It is fruitful to
think of a category as a \emph{proof-relevant preorder}, where
morphisms are thought of as proofs of ordering and the reflexivity and
transitivity constructions must additionally satisfy equations that
make them act like identity and composition. Then a functor is a
constructive version of a monotone function.
...
each set $R(c,d)$ as a set of ``heteromorphisms'' from $c$, an object
of $\cat C$ to $d$, an object of $\cat D$. Functoriality then defines
a notion of composition of heteromorphisms with homomorphisms in $\cat
C$ and $\cat D$. Thinking of categories as constructive preorders, a
profunctor is a constructive relation that is downward-closed in $\cat
C$ and upward-closed in $\cat D$, a kind of ``hetero-ordering''.
%
...
The simplest example of a profunctor is for any category $\cat C$,
$\text{Hom}_{\cat C} : \cat C \pto \cat C$ is a profunctor. On
preorders this is just the ordering itself. The hom profunctor is the
unit for a composition of profunctors $R \odot S$ which is defined as
a \emph{co-end}. The heteromorphisms of $(R \odot S)(c,e)$ are triples
$(d \in \cat D, f \in R(c,d), g \in S(d,e))$ modulo the equivalence
relation generated by $(d,f,g) ~ (d', f', g')$ if there exists $h \in
D(d,d')$ such that $h\circ f = f'$ and $g'\circ h = g$. The
composition of profunctors is a generalization of the composition of
relations, and the hom profunctor is analogous in the same way to the
diagonal, or equality relation in that it is the identity for this
composition.
For any profunctor $R : \cat C \pto \cat D$ and functors $F : \cat
C' \to \cat C$ and $G : \cat D' \to \cat D$, by composition we can
define a profunctor $R(F,G)$, the ``restriction'' of $R$ along $F$ and
$G$.

Profunctors are not explicitly defined in most introductory texts on
category theory but arise implicitly in formulations of universal
properties such a representable functors and adjunctions.
...
\item In \cref{sec:examples} we show how to use our syntax for
  synthetic category theory. Much of this is just rendering well-known
  2-categorical/double categorical constructions in our type theoretic
  syntax to demonstrate the ``naive''/synthetic character of proofs
  and constructions in SCT.
...
\item In \cref{sec:semantics}, we develop some basic model theory for
  ordered predicate logic, including a sound and complete notion of
  categorical model and sound interpretation in virtual equipments
  modeling ordinary, enriched and internal category theory.
...
\begin{figure}
  \begin{center}
    \begin{tabular}{ |c|c| } 
      ...
      1 & $1$ \\
      $\mathcal P X$ & $\mathcal P^+ X, \mathcal P^- X$ \\
      ...
      $R \wedge Q$ & $R \times Q$\\
      $\top$ & $1$\\
      $\forall x. P \Rightarrow Q$ & $\homrallXYtoZ {\alpha:\cat C} P Q, \homlallXYtoZ {\alpha:\cat C} P Q$ \\
      $\exists x. P \wedge Q$ & $\tensorexistsXwithYandZ {\alpha:\cat C} P Q$\\
      $x =_X y$ & $\punitinXfromYtoZ {\cat C} \alpha \beta$\\
      Proof $\forall \overrightarrow \alpha. R_1(\alpha_1,\alpha_2) \wedge \cdots \Rightarrow Q$ & Nat. Trans./Element $\alpha_1,x_1: R_1(\alpha_1,\alpha_2),\ldots \vdash t : Q$\\
      \hline
    \end{tabular}
  \end{center}
...
After these forms we have types and terms, which are for the most part
the same as in dependent type theory, except that we have universe
objects for classifying categories, functors, profunctors and natural
transformations.
%
Finally we have several forms of context which are used in the theory:
contexts $\Gamma$ of term variables with their types, contexts $\Xi$
of object variables with their categories, and contexts $\Phi$ of
...
\emph{sets}, that is, if we have a set $P$ parameterized by a
contravariant variable $\alpha : \cat C$ and a covariant variable
$\beta: \cat D$, then we can substitute objects $a : \cat C$ and $b :
\cat D$ for these variables $P[a/\alpha;b\beta]$. This generalizes the
ordinary precomposition of a relation by a function.  Semantically
this is the ``restriction'' of a profunctor along two functors, which
is just composition of functors if a profunctor is viewed as a functor
to $\Set$. Modeling this operation as a substitution considerably
simplifies reasoning using profunctors.
%
...
We introduce the connectives for defining categories in
Figure~\ref{fig:category-connectives}. We show the formation,
introduction and elimination rules, but additionally there are rules
for $\beta\eta$ equality and substitution, which are included in the
appendix.
%
First we introduce the additives: the unit category $1$ and product
category $\cat C \times \cat D$ with their usual introduction and
elimination rules.
%
...
\begin{figure}
  \begin{mathpar}
    \inferrule{}{\Gamma \vdash 1 \isaCat}\and
...
Next, we cover the connectives for the sets, i.e., constructions on
profunctors in Figure~\ref{fig:set-connectives}.
%
...
the unit type $\punitinXfromYtoZ {\cat C} {a_1}{a_2}$.
%
First, the unit set is our syntax for the profunctor of morphisms of a
category $\cat C$.
%
Its introduction and elimination rules are like those of an
\emph{ordered} variant of equality: the introduction rule is the
identity morphism (reflexivity) and the elimination rule is an
induction principle that is a variant of the J rule of equality
elimination: we can use a term of $s : \punitinXfromYtoZ {\cat C} a b$
by specifying the behavior when $s$ is of the form $\punitrefl \alpha$
in the form of a continuation $\alpha. t$. Again we elide all
$\beta\eta$ rules, which are standard. Next, the tensor product of
sets is a kind of combined existential quantifier and monoidal
product, which we combine into a single notation
...
%
Note that in the unit elimination, the continuation is typed with a
context consisting of only a single object $\alpha$, in contrast with
normal rules for a unit type in natural deduction for ordered linear
logic such as \cite{polakawpfenning}, which would split the context
$\Phi$ into $\Phi_l \jnctx \Phi_m \jnctx \Phi_r$ where $\Phi_m$ is
used by the scrutinee and $\Phi_l \jnctx \Phi_r$ is used by the
continuation. However, unlike in ordered linear logic, the $\Phi_l
\jnctx \Phi_r$ is not, necessarily well formed because there is no
reason in general that $\Phi_m$ has contravariant and covariant
dependence on the same category.
%
This is analogous to a similar situation in dependent type theory for
identity types: TODO: Dan please help with this explanation!!

Next, we introduce the contravariant ($\homlallXYtoZ \alpha R P$) and
covariant ($\homrallXYtoZ \alpha R P$) homs of sets, which are
different because we are in an orderd logic.
%
...
dependence. So as with linear logic, the cartesian unit and product
complicate the syntactic formulation of linearity: the unit
constructor ``uses'' all variables and the product ``uses'' all
...
\begin{figure}
  \begin{mathpar}
    ...
    \inferrule
    {d^-\Xi \isSmall\\\\
      \Gamma \pipe \alpha:\cat C; d^-\Xi\vdash R \\\\ \Gamma \pipe \alpha : \cat C; d^+\Xi\vdash P}
    {\Gamma \pipe \Xi \vdash \homlallXYtoZ {\alpha: \cat C} R P}

  \inferrule
  {d^+\Xi \isSmall\\\\ \Gamma \pipe d^+\Xi; \alpha : \cat C \vdash  R \\\\ \Gamma \pipe  d^-\Xi; \alpha : \cat C \vdash P}
  {\Gamma \pipe \Xi \vdash \homrallXYtoZ {\alpha:\cat C} R P}\\
...
%%       \pendallXdotY {\alpha_1} {\homrallXYtoZ {\alpha_2} {\punitinXfromYtoZ {\cat C} {\alpha_1}{\alpha_2}} {\harrapp Q {\alpha_1}{\alpha_2}}}}
    ...
    \inferrule
    {\Gamma\pipe \alpha:\cat C\vdash t : P[\alpha/\alpha^-;\alpha/\alpha^+]\\\\
     \Gamma\pipe\Phi \vdash s : \punitinXfromYtoZ {\cat C} A B}
    {\Gamma \vdash \punitelimkontZatABC {\alpha. t} A s B : P[A/\alpha^-;B/\alpha^+]}
...
Finally, we briefly describe the connectives for the
``meta-logic''. These include the standard rules for Martin-L\"of type
theory with Pi/Sigma and extensional identity types, whose rules we
elide as they are standard \cite{emltt}. We use extensional identity
types so that the description of models is simpler, but there is no
obstacle to including models with intensional identity types.
%
...
\section{Synthetic Category Theory in Ordered Higher-order Logic}

To demonstrate that \ohol{} provides a setting in which to carry out
synthetic category theory, we demonstrate some basic definitions and
theorems in category theory in \ohol{}. We emphasize throughout the
interpretations as tautologies in higher order-logic, as well as the
relation to ordered linear logic.

First, we fix some terminology: TODO, isomorphism of profunctors, anything else we'll need?
%% \begin{definition}{Isomorphism of Profunctors}
  
%% \end{definition}

We can witness the fact that all constructions are (pro-)functorial by
using the hom type $\punitinXfromYtoZ {\cat C}{\alpha}{\alpha'}$.
\begin{lemma}
  \begin{enumerate}
  ...
  \item Functoriality: for any $F : \varr {\cat C}{\cat D}$, $\homunary {\alpha_1} {\punitinXfromYtoZ {}{\alpha_1}{\alpha_2}} {\alpha_2} {\punitinXfromYtoZ {} {F(\alpha_1)}{F(\alpha_2)}}$.
   \item Profunctoriality: for any $R : \harr {\cat C}{\cat D}$ $\homtrinary {\alpha_1} {\punitinXfromYtoZ{} {\alpha_1}{\alpha_2}} {\alpha_2} {R{\alpha_2}{\beta_2}} {\beta_2} {\punitinXfromYtoZ{} {\beta_2} {\beta_1}} {\beta_1} {R\alpha_1\beta_1}$
  \end{enumerate}
\end{lemma}
...
\begin{lemma}[Naturality]
  ...
  terms
  \[ \alpha_1, f : \punitinXfromYtoZ {\cat C} {\alpha_1} {\alpha_2}, \alpha_2 \vdash \textrm{lcomp}(f,\pendappXtoY t {\alpha_2}) \quad \textrm{and} \quad \textrm{rcomp}(\pendappXtoY t {\alpha_1}, f) : \harrapp R {\alpha_1}{\alpha_2}\]
  These are both equal to $\punitelimtoYwithkontZ f {t}$.
\end{lemma}
...
Next, we have the ``Fubini'' theorems. These correspond to quantifier
re-ordering and associativity of conjunction and implication in
higher-order logic.
...
\begin{lemma}[{Fubini}]
  ...
  \end{enumerate}  
\end{lemma}
Note that the last item here is different from the others in that it
is an equivalence of sets rather than profunctors.

Next, we turn to basic universal properties. While there are many
equivalent definitions of a universal property in category theory, the
one most natural in \ohol{} is the notion of a functor representing a
profunctor, a mild generalization of the more common notion of
representable presheaf. We say a functor $F : \varr {\cat C}{\cat D}$
left represents a profunctor $R : \harr {\cat C}{\cat D}$ when
$\punitinXfromYtoZ {}{\beta}{F \alpha} \cong \harrapp R \alpha
\beta$. This is an ordered generalization of the notion of a relation
$R$ being the \emph{graph} of a function $F$. Due to the asymmetry,
there is a dual notion of a functor $G : \varr {\cat D}{\cat C}$
right-representing a profunctor $\punitinXfromYtoZ {\alpha}{G\beta}
\cong \harrapp R \alpha \beta$. Then $F$ and $G$ are adjoint functors
when $F$ left represents a profunctor that $G$ right-represents. This
generalizes the formulation that a function $g$ is an inverse to $f$
if its corresponding relation is the converse of the relation
corresponding to $f$.

The definition we gave for adjoint functors is the one in terms of
``hom-isomorphism''. The equivalent in terms of unit and co-unit is a
simple corollary of the Yoneda lemma, because for instance the unit
can be extracted from the transformation $\homunary \alpha
{\punitinXfromYtoZ {}{F \alpha}{\beta}} \beta {\punitinXfromYtoZ
  {}{\alpha}{G\beta}}$. The formulation in terms of unit and co-unit
generalizes the formulation of inverses that each composition is equal
to the identity.
%% \begin{lemma}[Formulations of Representability]
%%   The following are equivalent
%%   \begin{enumerate}
%%   \item An isomorphism between $\punitinXfromYtoZ {}{F \alpha}{\beta}$ and $\harrapp R \alpha \beta$
%%   \item A term $\pendallXdotY \alpha {\harrapp R \alpha {F \alpha}}$
%%     such that the induced morphism from $\punitinXfromYtoZ {}{F
%%       \alpha}{\beta}$ to $\harrapp R \alpha \beta$ is an isomorphism.
%%   \end{enumerate}
%% \end{lemma}

Next, we turn to limits, or more generally, Kan extensions.
%
Given functors $J : \varr {\cat C}{\cat C'}$ and $F : \varr{\cat
  C}{\cat D}$, a right Kan extension is functor $\ran J F : \varr{\cat
  C'}{\cat D}$ such that the composition $\ran J F \circ J$ has a
universal natural transformation to $F$.
\begin{lemma}
  A right Kan extension of $F$ along $J$ is a functor $\ran J F$ that
  right-represents the profunctor $\homrallXYtoZ {\alpha} {\punitinXfromYtoZ {} {\alpha'}{J \alpha}}{\punitinXfromYtoZ {} \beta {F \alpha}}$
\end{lemma}
Viewing this in higher-order logic, this says that $\beta = (\ran J
F)\alpha'$ if and only if $\forall \alpha. \alpha' = J \alpha
\Rightarrow \beta = F \alpha$. If $J$ is surjective this says that
$\ran J F$ composed with $J$ is equal to $F$, but if $J$ is not
surjective then this is impossible to satisfy for ordinary
functions. Kan extensions generalize limits in that the limit of a
diagram $F : \varr {\cat C}{\cat D}$ is the same as a right Kan
extension along the unique $! : \varr{\cat C}{1}$.

Then we can prove the famous theorem that right adjoints preserve
right Kan extensions, and so in particular, limits.
\begin{theorem}
  If $\ran J F$ is a right Kan extension and $G : \varr{\cat D}{\cat
    E}$ has a left adjoint $L$, then $G \circ (\ran J F)$ is the right
  Kan extension of $G \circ F$ along $J$.
\end{theorem}
\begin{proof}
  \begin{align*}
    \punitinXfromYtoZ {} {\gamma} {G (\ran J F \alpha')}
    &\cong \punitinXfromYtoZ {}{L \gamma} {\ran J F \alpha'}\\
    &\cong \homrallXYtoZ {\alpha}{(\punitinXfromYtoZ {}{\alpha'}{J\alpha})} {\punitinXfromYtoZ {}{L \gamma}{F \alpha}}\\
    &\cong \homrallXYtoZ {\alpha}{(\punitinXfromYtoZ {}{\alpha'}{J\alpha})} {\punitinXfromYtoZ {}{\gamma}{G (F \alpha)}}
  \end{align*}  
\end{proof}
...
and predicate logic in certain\emph{virtual double categories}
\cite{leinster,crutwell-shulman}. We briefly recount the structure
present in a virtual double category, but see \citet{crutwell-shulman}
for a precise definition.
The following definition is
equivalent to the formulation as a ``normalized $T$-monoid'' in
\citet{crutwell-shulman}, and we direct the reader there for the full
description.
%
...
% https://q.uiver.app/?q=
\[\begin{tikzcd}
	{C_0} & \cdots & {C_{n}} \\
	& \phi \\
	{D_0} && {D_1}
...
\end{tikzcd}\]
  ...
  \to C$ and $g : D' \to D$ there is a chosen profunctor $R(f,g) : C'
  \pto D'$ with a cartesian 2-cell to $R$ framed by $f,g$ and these
  chosen cartesian lifts are functorial in $f,g$.  A functor of
  virtual double categories preserves split restrictions if it
  preserves these specified cartesian lifts.  
\end{definition}

Each component of this definition has a direct correspondence to a
syntactic structure in \ohol{}. The category $\mathcal V_o$ models the
category and functor judgments. The set $\mathcal V_h$ models the
profunctor judgment. A composable string $R_0 \cdots R_n$ models the
profunctor contexts. The 2-cells correspond to the natural
transformation judgment where we have taken the restriction $S(F,G)$
of the codomain.

To model the dependent type theory and fact that the \ohol{} judgments
are all under a dependent type theory context, we use a variation on
Lawvere's notion of \emph{hyperdoctrine} for modeling predicate logic:
\begin{definition}[Hyperdoctrine of Virtual Double Categories with Restrictions]
  A hyperdoctrine of virtual double categories with restrictions is a
  pair of a category with families $\mathcal C$ and a functor $L :
  \mathcal C^o \to \text{vDbl}_r$ to the category of virtual double
  categories with split restrictions and restriction-preserving
  functors.
\end{definition}
Categories with families $\mathcal C$ model dependent type theory
\citep{cwf} and for each semantic context $\Gamma$, $L(\Gamma)$ models
the \ohol{} judgments in context $\Gamma$, with the functoriality
modeling the fact that all of these judgments admit a well-behaved
action of substitution.

\begin{construction}[Soundness and Completeness]
  For any choice of connectives, the syntax of parameterized \ohol{}
  defines a hyperdoctrine of VDCrs that is an initial object in the
  category of hyperdoctrines of VDCrs with chosen universal
  constructions and functors preserving those universal constructions.
\end{construction}
\begin{proof}
  Most of the details of the proof are straightforward. In the
  appendix we demonstrate the syntactic elements of the proof which
  are that the syntax itself is a hyperdoctrine of VDCrs, i.e., that
  the notions of substitution are appropriately functorial.
\end{proof}
...
notion of semantics in (enriched, internal) category theory.

First, we can extend any model of \ohol{} to a model of \ohol{}
parameterized by dependent type theory as follows:
\begin{construction}
  Given a virtual double category with restrictions $\mathcal V$, we
...
TODO: find a reference that proves things have all the UMPs we want
lol.

\section{Discussion}

\paragraph{Calculi for formal category theory}

In their book on $\infty$-category theory, Riehl and Verity use the
formal language of virtual equipments to prove results valid for
$\infty$-categories without concrete manipulation of model
categories. They formalize this language using Makkai's FOLDS
framework of first-order logic with dependent sorts. The approach is
similar in spirit to our \ohol{}, and has the same models. The main
difference is that their theory is built on top of first-order logic,
whereas ours is built on a custom-made variant of higher-order logic
over a (unary) simple type theory. For this reason, statements in
their theory are quite complex as they have to be encoded into
first-order logic, whereas we argue our type-theoretic approach
formalizes informal arguments much more directly, as shown in
Section~\ref{sec:examples}.
%
An informal syntax for manipulating profunctors involving ends and
coends, called the coend calculus is closely related, an extension of
\ohol{} to treat profunctors of many variables of different variances
should provide a formal treatment for the coend calculus
\citet{coend-calculus}.

Another approach to formal category theory is to develop a
\emph{directed dependent type theory}, a type theory where the
identity type is interpreted as morphisms in a (possibly
$\infty$-)category. Research on directed dependent type theories is
ongoing \cite{riehl-shulman,north}. Directed dependent type theories
...
virtual double category of categories, functors, profunctors and
natural transformations. An abstract structure along
these lines is a \emph{Cartesian bicategory}\footnote{which is not the
same as a bicategory with products/finite limits, for which a syntax
was developed \citet{fioresaville}} is given in
\citet{carboni-walters,carboni-walters-someonelese}. 
Some preliminary work on a proof system for Cartesian bicateogires was
presented at \citet{jonasfreycartesianbicategories}. This bears strong
similarities to our calculus VETT, except that (1) their profunctors
may have 0, 1 or more covariant or contravariant variables but (2)
they do not have a term syntax like our natural transformation syntax
and (3) they do not have functors and restriction of profunctors by
functors (since this structure is not present in a Cartesian
bicategory). Semantically, Cartesian bicategories are somewhat less
general than virtual equipments because in general the set of
functors between two categories can only be recovered from the set of
profunctors if the codomain category is Cauchy complete, in which case
functors are equivalent to adjoint pairs of profunctors.
...
are not presented as having co-variant and contravariant variables,
and the equipment structure is modeled by explicit coercions rather
than by syntactic substitution of vertical arrows in horizontal
arrows. Another similar calculus is System P \citep{systemP} which is
an internal language of \emph{reflexive graph categories}, which are
like double categories without horizontal composition. 

\subsection{Category Theoretic Extensions}
...
Our definition of a double category with displays is suggestive of a
simple generalization. A double category is a monoid object in the
double category of $Span(Cat)$, and a double category with displays
furthermore uses certain diagrams involving profunctors, which also
form a double category $Prof$. This suggests that the algebraic
structure of a double category with displays is naturally modeled in
some kind of \emph{triple category}, modeling the three types of
morphisms: functors, profunctors and spans of functors.
