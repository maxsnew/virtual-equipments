...
\begin{definition}
  ...
  \begin{enumerate}
  \item The functor $(s,t) : C_d \to C_0$ is a split fibration, with
    restriction operation written as $(f,g)^*R$.
...
\begin{definition}
  ...
  \item with a terminal object $1 \in \textrm{Ctx}$
  \item A presheaf $\textrm{Ty} : \Set^\textrm{Ctx}$ of types in context.
  \item A presheaf $\textrm{Tm} : \Set^{\int \textrm{Tm}}$ of terms of
    a given type.
...
\begin{definition}
  ...
  category with families and let $\mathcal V$ be a DCD internal to
  $\Set^\textrm{Ctx}$. Then
  \begin{enumerate}
  \item $\mathcal V$ objects are representable as types if there is a
    type given by a section $\textrm{Ob} \in \textrm{Ty}(1)$ with a
    natural isomorphism
    \[ \textrm{Tm}(!_\Gamma^*\textrm{Ob}) \cong \textrm{Ob}(\mathcal V)_\Gamma \]
  \item $\mathcal V$ vertical arrows are representable as types if
    there is a type
    \[ \textrm{Tm}(((\gamma,\lceil a\rceil),\lceil b\rceil)^*\textrm{Functor} \cong
    \textrm{VArr}(\mathcal V_{\Delta})(a,b)
    \]
    and similarly horizontal displays are representable as types if
    \[ \textrm{Tm}(((\gamma,\lceil a\rceil),\lceil b\rceil)^*\textrm{Profunctor} \cong
    \textrm{Disp}(\mathcal V_{\Delta})(a,b)
    \]
...
$\Set^\Set$ by taking the presheaf of objects, vertical arrows, etc as
...
\begin{definition}[Powers of Virtual Equipments]
  ...
  \item A 2-cell from $\Phi$ to $R$ is a family $\Pi_{x\in X} \mathcal
    V^X_h(\Phi x,R x)$.
  \end{enumerate}
  All operations are simply lifted pointwise.
\end{definition}
...
\begin{definition}[Internal Logic]
  ...
  \item Restriction by a function $\gamma : \Delta \to \Gamma$ is
    given pointwise.
  \item The representing object for objects is simply
    $\textrm{Ob}_\Gamma = \textrm{Ob}(\mathcal V)^\Gamma$ and
    similarly for the other universe objects.
  \end{enumerate}
\end{definition}
