...
To simplify our initiality proof, we work with an \emph{essentially
algebraic} presentation of the notion of VE hyperdoctrine. As with all
GATs, this has an initial model, which is a kind of ``combinator''
syntax with explicit substitution for VE hyperdoctrines. Then we show
that our type theoretic syntax is equivalent to the combinator syntax
by a simple inductive ``simplification'' algorithm. We conjecture that
a full normalization proof is possible, which would establish useful
conservativity results for the calculus.

The theory has sorts Ctx, Subst, Tm, Ty, Cat, Fun, Prof, ProfCtx and
Trans.
