...
\begin{abstract}
  ...
  transformations. By restricting the type theory to an ordered
  linear restriction of higher-order logic, we guarantee that all
  mappings between categories are functorial and all transformations
  are natural by construction, with no separate proof
  necessary. Important category theoretic proofs such as the Yoneda
  lemma and Co-yoneda lemma become simple type theoretic proofs about
  the relationship between unit, tensor and (ordered) function types,
  and can be seen to be ordered refinements of theorems in
  intuitionistic higher-order logic.

  We give a sound and complete notion of model of our type theory in a
  structure we call a hyperdoctrine of pro-arrow equipments with
  displays, which are known to model enriched and internal category
  theory. This means that while the proofs in our type theory look
  like standard set-based arguments, the substructural syntactic
  restrictions ensure that the proofs carry over to enriched and
  internal settings as well.
\end{abstract}
...
quickly discovered that these algebraic structures were common in
logic and computer science, and that formal systems like type theories
and programming languages can be modeled by a sound and complete
notion of semantic model given by some notion of structured
category\cite{lawvere,goguen,lambek,moggi}.
%
...
%
These seminal works have led to a fruitful field of research applying
such categorical logic techniques to the design of new type theories
based directly on category-theoretic semantics.
%
...
category theory.
%
Consider the analogous situation in differential calculus. When a
physicist writes down a function, they do not expand out the
...
In this work we show that these algebraic formulations of category
theory can be given a type-theoretic syntax that allows us to make
``na\"ive'' set theoretic constructions for categories, but by
adhering to a certain syntactic discipline provide correct arguments
for not just ordinary category theory but enriched and internal
categories as well.
%
The type theory, which we call SCT, is a ``synthetic'' theory of
categories, functors, profunctors and natural transformations.
%
This provides a formulation of category theory in which profunctors
play a primary role, most recognizable in ordinary category theory in
the prominence of ``hom isomorphism'' formulations of universal
properties.
%
The semantics of SCT is given by a slight generalization of virtual
double categories \cite{} that we call double categories with
displays.
%
Additionally, to define signatures and parameterize theorems like the
Yoneda lemma, SCT includes an ordinary dependent type theory over it
which includes usual $\Pi,\Sigma,\Id$ types.
%
In an implementation of SCT, this dependent type theory might be the
host language itself.

In this work, we focus on the syntax and semantics of SCT, but the
ultimate goal is for this theory to serve as the foundation for a
framework in which we can more easily study and program with new type
theories. This falls in the tradition of \emph{logical frameworks}
such as the LF system. To provide evidence for the feasibility of this
approach, we show that we can model structures used in recent work on
modal dependent type theory. This shows that an implementation of SCT
would provide at least a toy prototype implementation/type checker for
modal dependent type theory. This can be summarized in the slogan
\begin{quote}
  category theory as a logical framework
\end{quote}
that is, that a syntax for category theory can be used as a logical
framework for type theories with semantics in structured categories.
%
If implemented in a proof assistant this would also provide with it
the soundness of the syntax in any model.
%
The emphasis on profunctors in SCT fits nicely with the
\emph{judgmental} approach of Per-Martin L\"of and suggests the
following slogan
\begin{quote}
  profunctors as judgments, natural transformations as derivations
\end{quote}
because we observe that the ``derivation tree'' style of writing
judgments corresponds closely to the type structure of profunctors and
natural transformations in SCT, and it should be simple to implement a
program that converts formal natural transformations into derivation
tree syntax.

This work is merely a first step in this direction, as there are still
three major components of category theory: opposite categories,
functor categories and ``universes'' such as the category of sets or
categories that are not nicely axiomatizable in the current system.
%
Nontheless, a great deal of category theory can be carried out in this
more limited setting, and we can still take advantage of duality at a
meta level.

\subsection{Ordered Logic of Functions and (Constructive) Relations}

Our logic is based on four basic notions: categories, functors,
profunctors and (multi-argument) natural transformations.
%
These generalize the basic notions already present in formulations of
higher-order logic: sorts, functions, relations and proofs.
%
In fact, one way to view our logic is that it is an \emph{ordered},
\emph{constructive} higher-order logic

While profunctors are not typically employed in introductory category
theory texts, 

\subsection{Category theory with Profunctors}

First, we review the 4 basic notions of our theory of categories in
the traditional formulation. A category $\cat C$ is a set of objects
$\cat C_0$ with a span of arrows $\cat C_0 \leftarrow \cat C_1
\rightarrow \cat C_0$ that pick out the source and target of each
arrow, such that there are composition and identity arrows with
appropriate laws. A functor $F : \cat C \to \cat D$ consists of a
function on objects $F_0 : \cat C_0 \to \cat D_0$ and a function on
morphisms $F_1 : \cat C_1 \to \cat D_1$ that is compatible with source
and target and preserves identity and composition.

A profunctor $R$ from $\cat C$ to $\cat D$ is a span $\cat C_0
\leftarrow R \rightarrow \cat D_0$ that is compatible with
\emph{left}-composition by $\cat C$ and \emph{right}-composition by
$\cat D$. If we think of categories as generalizations of (ordered)
sets then functors generalize (monotone) functions and profunctors
generalize (monotone) \emph{relations}. We think of an element $r \in
R$ with $s(r) = c$ and $t(r) = d$ as a ``proof'' that $c R d$ or some
kind of ``heteromorphism'' from $c$ to $d$. The compatibility with
composition says that these relations are down-closed in $C$ and
up-closed in $D$, or that heteromorphisms can be composed with
homomorphisms.
%

%% Embracing the analogy ``profunctors as judgments'' we can view
%% elements of a profunctor $r \in R$ with $s(r) = A$ and $t(r) = B$ in
%% type-theoretic notation
%% \[ \inferrule*[right={r}]{~}{A \vdash_R B}\]
%% Noting that every category $\cat C$ has a hom profunctor $Hom_C : \cat
%% C \pto \cat D$, we get for each category a judgment
%% \[ \inferrule*{}{A \vdash_{Hom_C} A'} \]
%% and the category operations provide us with rules for use of the assumption:
%% \[ \inferrule*[right=id]{}{A \vdash_{Hom_C} A} \]
%% and cut
%% \[ \inferrule*[right=cut]{A \vdash_{Hom_C} A' \and A' \vdash_{Hom_C} A''}{A \vdash_{Hom_C} A''} \]
%% satisfying unit and associativity equations.
%% %
%% An arbitrary profunctor $R$ supports a kind of ``bi-cut''
%% rule\footnote{alternatively, this can be defined as 2 cuts: left and
%% right, with an associativity law}:
%% \[ \inferrule*[right=bicut]{A \vdash_{Hom_C} A' \and A' \vdash_R B' \and B' \vdash_{Hom_D} B}{A \vdash_R B} \]
%% That satisfies similar bi-unit and bi-associativity laws.

\subsection{Double Categories and Pro-arrow Equipments}

We can abstract the algebraic structure of categories, functors,
profunctors, and natural transformations as a \emph{double category}.
%
A double category is a generalization of categories where we have a
collection of objects $C_0$, and \emph{two separate} notions of
morphism, which we call vertical $C_v$ and horizontal $C_h$ over the
same objects $C_0$ which each have their own identity and composition,
and finally a notion of 2-cell that is most easily described 
...
We address both of these issues by developing an alternative
\emph{type theoretic syntax} for virtual equipments.
%
In this syntax the ``multi-2-cell'' above instead becomes a natural
transformation judgment
\[ \Phi \vdash t : S[F(x);G(y)] \]
where the composable string of input arrows is modeled by an ordered,
linear context $\Phi$ and output $S$ and frames $F,G$ are combined in
the output type using the restriction operation of an equipment.
%
...
%% \subsection{Applications to Generalized Type Theory}

%% While we mostly focus on the semantics in category theory, due to the
%% tight connection between category theory and the semantics of type
%% theory, there are direct applications to the study of ``abstract type
%% theory'' as well. We demonstrate how we can model single-variable
%% simple type theories with products and coproducts within SCT. This is
%% very much in the spirit of logical frameworks \cite{TODO}, since we
%% can faithfully model the terms of an object theory inside of our
%% metatheory. Since we have models of our metatheory in virtual double
%% categories, we establish sound semantics as well. Put succinctly,
%% category theory itself is the logical framework for the study of type
%% theories.
...
\item In \cref{sec:examples} we show how to use our syntax for
  ...
  and constructions in SCT. We additionally give some examples of how
  this can be applied to model a recent formulation of modal dependent
  type theory.
...
We have aimed that the syntax and examples sections should be
accessible to anyone familiar with basic category theory and
willingness to understand profunctors and (co-)ends.
%
The semantics section is by necessity more advanced and may be skipped
or skimmed by readers who are willing to trust the mathematical
...
logic with our category logic. Since category logic is an ordered
variant of higher-order logic, some notions splinter into
multiple. For instance, covariant and contravariant presheaf
...
which combined with implications and conjunctions.
...
\section{Semantics}
Next, we develop the basics of the model theory for \ohol{}. Being a
2-level type theory, \ohol{} has models that are a kind of
\emph{hyperdoctrine}. First, a model must include a model of
intuitionistic type theory to interpret the type-level context
$\Gamma$ and corresponding type/term judgments. Next, there should be
a functor from the category of semantic contexts and substitutions to
a category of models of ordered higher-order logic.
...
\paragraph{Calculi for 2-dimensional category theory}
...
Our type-theoretic syntax for virtual double categories is intended to
be a formal notation for the manipulation of profunctors. As such it
has similarities to informal syntax for manipulating profunctors
involving ends and coends, such as the coend calculus of
...
Double-cat-like syntax:
Gradual Type Theory, System P of parametricity, cubical stuff?

\subsection{Directed Type Theory}
...
Grants stuff...? The authors would like to thank Emily Riehl, Mike
Shulman,...for helpful feedback on this work
