...
\begin{abstract}
  ...
  (ordered) function types. We demonstrate that this type theory can
  be used as a kind of logical framework in which we can define common
  categorical structures such as categories with families that serve
  as models of type theories.
...
parametricity and gradual typing.
...
using category-like algebraic structures such as 2-categories,
bicategories, (virtual) double categories and pro-arrow
equipments\cite{}.
%
...
One final multiplicative we can define is the \emph{category of
elements} of a profunctor. Given a profunctor $\alpha:\cat
C;\beta:\cat D \vdash R$, the ``category of elements'' of $R$ is a
category $\Sigma_{\alpha:\cat C;\beta:\cat D}. R$. In detail, its
objects are triples $(a:\cat C, r : R, b : \cat D)$ and morphism from
$(a,r,b)$ to $(a', r', b')$ consists of a pair of morphisms $g : b \to
b'$ and a backwards $f : a' \to a$ such that $r' = g\circ r \circ f$.
%
...
category $\exists \alpha;\beta. \punitinXfromYtoZ {\cat
...
\emph{category} rather than a profunctor. Also note that while similar
to the tensor in that we use existential notation, the graph of a
profunctor is a \emph{negative} multiplicative, analogous to a
$\Sigma$-type. The universal property says that a functor from $\cat
E$ into the graph $\exists (\alpha; \beta). R$ is equivalent to a
