...
\title{An Ordered Logic for Category Theory}
...
\begin{abstract}
  ...
  We develop a sound and complete notion of categorical model and give
  a model construction that shows that the theorems are valid in
  enriched and internal categories. This means that while the proofs
  in our type theory look like standard set-based arguments, the
  substructural syntactic restrictions ensure that the proofs carry
  over to enriched and internal settings as well.
\end{abstract}
...
quickly discovered that these categorical structures were common in
logic and computer science, and that formal systems like logics, type
theories and programming languages can be modeled by sound and
complete semantic models given by notions of structured
category\cite{lawvere,goguen,lambek,moggi}, a phenomenon sometimes
referred to as the Curry-Howard-Lambek correspondence
...
categorical logic, where
new type theories are designed based directly on category-theoretic
semantics.
%
...
\emph{ordered predicate logic}, that can be soundly interpreted in
virtual equipments.
%
In ordered predicate logic, the sorts are interpreted as categories,
functions as functors, relations as profunctors and implications of
relations as natural transformations.
%
...
Ordered predicate logic is a kind of \emph{ordered},
\emph{constructive} variant of predicate logic over a unary type
theory.
%
...
%
We show that a great deal of fundamental results in category theory
are already expressible in this fragment.
...
is too restrictive to encode.

The dependent type theory layer also has an additional purpose: since
we can quantify over the structures in ordered predicate logic, we can
additionally encode \emph{signatures} for ordered predicate logic as
...
for parametricity and LSLR for step-indexed logical relations proofs
\cite{abadi-plotkin,lslr}.
%
Many \emph{binary} logical relations are based on ordered structures,
such as logical relations for approximation in non-deterministic or
partial languages or logical relations for graduality in gradual
typing.
%
We conjecture that some extension of the ordered predicate logic
defined here will be useful for describing these relations as well.
