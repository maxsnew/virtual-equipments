...
\vdash J$ is interpreted as simply a family of $J$-data parameterized
...
\begin{definition}
  A strict double category is a monoid in the double category $Span(Cat)$.
\end{definition}
...
a ``singleton'' context $\alpha:\cat C,x:R,\beta:\cat D$.
...
associativity/identity equations for a DCD are simply those of a
double category.
...
\begin{definition}
  A hyperdoctrine of equipment with displays (HED) is a pair of a
  category with families $Ctx$ modeling $\Sigma,\Pi,\textrm{Id}$ and a
  DCD internal to $\Set^{Ctx^o}$ with categories, functors, displays
  and unary cells representable as types.
\end{definition}

Again this notion is straightforwardly, if laboriously, interpretable
as a GAT.
...
\begin{definition}{Indexed Virtual Equipment}
  A VE-hyperdoctrine is a pair of a category with families $(C,
  \textrm{Ty}, \textrm{Tm})$ and a functor $\cat C^{op} \to VE$. A
  morphism of VE-hyperdoctrines
  \[(F,\phi) : (\cat C, \mathcal V : \cat C^{o} \to VE) \to (\cat D, \mathcal U : \cat D^{o} \to VE)\]
  is a pair of a functor of categories with families $F : \cat C \to
  \cat D$ and a natural transformation $\phi : \mathcal V
  \Rightarrow\mathcal U \circ F^{o}$.
\end{definition}
...
\begin{corollary}
  SCT is initial in the category of HDCDs.
\end{corollary}
\begin{proof}
  Since Comb is initial, the composite $i \circ \eta$ is the identity,
  and as above the composite $\eta \circ i$ so SCT is isomorphic to
  Comb, and therefore it is also initial.
\end{proof}

\subsection{Example Models}

Our main intended models are when the indexing category is $\Set$ and
the hyperdoctrine is given by families of objects, arrows etc, of a
fixed virtual equipment $\mathcal V$, indexed by the set
$\sem\Gamma$. We can formalize this as the ``internal logic'' of the
virtual equipment $\mathcal V$.
...
\begin{definition}[Powers of Virtual Equipments]
  ...
  \item Vertical morphisms $\mathcal V^X_v(A,B)$ are functions
    $\Pi_{x\in X} V_v(A x,B x)$. With pointwise composition, identity.
  \item Horizontal morphisms $\mathcal V^X_h$ are functions $\Gamma \to \mathcal V_h$, with
    $(d^\pm R)x = d^\pm (Rx)$.
  \item Restriction of $R$ along $f,g$ is given by $(R(f,g))x = (R x)(f x,g x)$.
  \item A 2-cell from $\Phi$ to $R$ is a family $\Pi_{x\in X} \mathcal
    V^X_h(\Phi x,R x)$ with composition given pointwise.
  \end{enumerate}
\end{definition}
...
\begin{definition}[Internal Logic]
  ...
  \begin{enumerate}
  \item The indexing cateogry is $\Set$.
...
\paragraph{Calculi for Presheaves/Profunctors}

David Jaz Myers String Diagrams
...
\paragraph{Category Theoretic Extensions}
  ...
  \end{align*}
    \item When $\psi=\psi,t/x,a/\alpha$
      \begin{align*}
        ...
        &= (\phi[\psi'] \jnctx \psi[\psi']),t[\psi'_2]/x,a[d^+\psi'_2]/\alpha\\
        &= \phi[\psi'] \jnctx (\psi[\psi'],t[\psi'_2]/x,a[d^+\psi'_2]/\alpha)\\
      \end{align*}
  \end{enumerate}
\end{lemma}
