...
constructors we choose) presents the initial indexed virtual equipment
(with the appropriate universal properties). To start we define
appropriate categories of equipments.

\begin{definition}{Indexed Virtual Equipment}
  A VE-hyperdoctrine is a functor $\cat C^{op} \to VE$, and a morphism
  of VE-hyperdoctrines is a natural transformation.
\end{definition}
...
\begin{definition}[Internal Logic]
  ...
  \item Given $\Gamma \in \Set$, the virtual equipment $\mathcal
    V^\Gamma$ is the ``power'' of $\mathcal V$ by $\Gamma$:
    \begin{enumerate}
    \item Objects are functions $\Gamma \to \mathcal V_0$.
    \item A vertical morphism from $O$ to $O'$ is a function $\Pi_{\gamma\in\Gamma} V_v(O\gamma,O'\gamma)$.
    \item A horizontal morphism is a function $\Gamma \to \mathcal V_h$. This satisfies
    \item A 2-cell from $R$ to $S$ is a family $\Pi_{\gamma\in\Gamma} \mathcal V_h(R\gamma,S\gamma)$.
    \end{enumerate}
    Compositions and restrictions are given pointwise.
