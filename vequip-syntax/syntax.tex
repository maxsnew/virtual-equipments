...
\begin{definition}[Internal Category]
  ...
  \item Identity arrows
    $C_r : \forall \alpha :  C. H \alpha \alpha$
  \item Composition, which we can express several different ways using
    a mix of covariant/contravariant homs and tensors.
    \[ C_t^\triangleright : \forall \alpha :  C. \forall \beta. (H \beta \alpha) \triangleright \forall \gamma. (H \gamma \beta) \triangleright H \gamma \alpha \]

    or

    \[ C_t^\triangleleft : \forall \alpha. \forall \beta. (\forall \gamma. H \alpha \gamma \triangleleft H \beta \gamma) \triangleleft H \alpha \beta
    \]

    or using tensors and covariant homs

    \[ C_t^\odot : \forall \alpha. \forall \gamma. (\exists \beta. H\alpha\beta\odot H\beta\gamma)\triangleright H\alpha \gamma \]

    or using tensors and contravariant homs

    \[ C_t^\odot : \forall \gamma. \forall \alpha. H\alpha\gamma \triangleleft (\exists \beta. H\alpha\beta\odot H\beta\gamma)\]
  \item Satisfying Unitality, i.e., for any $\Phi \vdash f : H A B$,
    we'd like $f \circ r(A) = f$. We can formalize this using an
    equality of two profunctor transformations of type $\forall
    \beta. \forall \alpha. H \alpha \beta \triangleright H \alpha
    \beta$ (or $\forall \alpha. \forall \beta. H \alpha \beta
    \triangleleft H \alpha \beta$), one of which is the identity and
    the other of which composes with a reflexivity morphism.

    \[ (\lambda \beta. \lambda^\triangleright \alpha. f:H \alpha \beta. f)
    =  (\lambda \beta. \lambda^\triangleright \alpha. f:H \alpha \beta. (\alpha, f)\triangleright (\beta, r\beta) \triangleright (C_t^\triangleright \beta))
    \]

    or expressed using $\triangleleft$, 
    \[
    (\lambda \alpha. \lambda^\triangleleft \beta. f:H\alpha\beta. f)
    =
    (\lambda \alpha. \lambda^\triangleleft \beta. f:H\alpha\beta. C_t^\triangleleft \alpha \triangleleft (\beta, f) \triangleleft (\beta, r \beta)) \]

    or expressed using $\odot$

    \[
    (\lambda \alpha. \lambda^\triangleleft \beta. f:H\alpha\beta.
  ...
  \item And Associativity, which we can express in several ways, but
    ...
    two terms of type
    \[ \forall \delta. \forall \alpha. (\exists \beta. H\alpha \beta \odot (\exists \gamma. H\beta\gamma \odot H \gamma \delta))\triangleright H\alpha\delta\]
    or
    \[ \forall \alpha. \forall \delta. H\alpha\delta\triangleleft (\exists \beta. H\alpha \beta \odot (\exists \gamma. H\beta\gamma \odot H \gamma \delta)) \]

    The equality would be between the terms
    \[ \lambda \alpha. \lambda \delta, fgh.
    \textrm{ind}(fgh, f : H \alpha\beta. \beta. gh. C_t^\odot \delta \triangleleft (\alpha, (f, \beta, C_t^\odot \gamma \triangleleft (\beta, gh))) )
    \]
    and
    \[ \lambda \alpha. \lambda \delta, fgh.
    \textrm{ind}(fgh, f:H \alpha\beta. \beta. gh.
    \textrm{ind}(gh, g:H\beta\gamma. \gamma. h:H\gamma\delta.
...
\begin{definition}[Internal Functor]
  ...
  \item That preserves identity. This is given by equality between the
    ...
    and
    \[ \lambda \alpha. F_1 \alpha \triangleleft (\alpha, C_r \alpha) \]
  \item And preserves composition. This is given by an equality between the following terms of type
    ...
    F_1 {F_0 \alpha} \triangleleft (C_t^\odot \alpha \triangleleft ((f,\beta,g), \gamma), F_0 \gamma)
    )\]
...
\begin{definition}[Internal Profunctor]
  ...
  \item A $C$-composition action:
    $R_l : \forall \alpha. \forall \beta. R_s \alpha \beta \triangleleft (\exists \alpha'. C_1 \alpha \alpha' \odot R_s \alpha' \beta)$
  \item $D$-composition:
    $R_r : \forall \alpha. \forall \beta. R_s \alpha \beta \triangleleft (\exists \beta'. R_s \alpha \beta' \odot D_1 \beta' \beta)$
  \item Satisfying $\cat C$ unitality:
    $R_l (C_r(\alpha), r) = r$
...
Since we have profunctors as a primitive notion, we can most easily
discuss universal properties stated in terms of profunctors and
representability as a functor, rather than other formulations (adjoint
functors, representable presheaves, kan extensions).

First, we have the ``algebraic'' notion of a functor representing a
profunctor, i.e., when a profunctor $R : C\to D$ is equivalent to
$Hom_D(-,G=)$.
\begin{definition}[Right Representability]
  ...
  \begin{enumerate}
  \item A functor $G : \varr C D$
  \item An ``introduction rule'' $I : \forall \alpha. \forall
    \beta. (Q \alpha \beta) \triangleright (\alpha \to_{D} G \beta)$
    that is a homomorphism of internal profunctors.
  \item A homomorphism $I^{-1} : \forall \alpha. \forall
    \beta. (\alpha \to_{D} G \beta) \triangleright (Q \alpha \beta)$.
  \item Such that $I, I^{-1}$ are inverses in that:

    \[\lambda \alpha. \lambda(\beta, f). f = \lambda \alpha. \lambda(\beta, f). I \alpha \triangleleft (\beta, I^{\-1} \alpha \triangleleft (\beta, f))\]
    and similarly the opposite composition is the identity.
  \end{enumerate}
\end{definition}
...
\begin{definition}[Universal Morphism]
  A profunctor $Q : \harr C D$ is representable when there exists
  \begin{enumerate}
  \item A function on objects $G : \varr C D$
  \item An ``introduction rule'' $I : \forall \alpha. \forall
    \beta. (Q \alpha \beta) \triangleright (\alpha \to_{D} G \beta)$
    (with no homomorphism conditions).
  \item An universal morphism $\epsilon : \forall \beta:D. Q(G \beta)
    \beta$.
  \item Such that composition with $\epsilon$ is an inverse to $I$.
  \end{enumerate}
\end{definition}
