...
\begin{definition}[Internal Category]
  ...
  \item Composition, which we can express several different ways using
    ...
    \[ C_t : \forall \alpha. \homrallXYtoZ \beta {C_1 \alpha \beta} {\homrallXYtoZ \gamma {C_1 \beta \gamma} {C_1 \alpha \gamma}}\]
  ...
  \item Satisfying right unitality, i.e., for any $\Phi \vdash f :
    {C_1} A B$, we'd like $f; r(B) = f$. We can formalize this
    using an equality of two transformations of type $\forall
    \alpha. \homrallXYtoZ \beta {C_1 \alpha \beta} {C_1 \alpha \beta}$

    \[ \lambda \alpha. \homrlambdaXatYdotZ f \beta {\homrappXtoYatZ {\homrappXtoYatZ {C_t \alpha} {f} \beta} {C_r \beta} \beta}    
    = \lambda \alpha. \homrlambdaXatYdotZ f \beta f \]
    ...
    sequence of $\lambda$s that start such equalities.

  \item and left unitality (an equality of the same type as right unitality):

    \[ f = \homrappXtoYatZ {\homrappXtoYatZ {C_t \alpha} {C_r \alpha} \alpha} f \beta \]
  \item and associativity, which is an equality at type
    \[ \forall \alpha. \homrallXYtoZ \beta {C_1 \alpha \beta} {\homrallXYtoZ \gamma {C_1 \beta \gamma} {\homrallXYtoZ \delta {C_1 \gamma \delta} {C_1 \alpha \delta}}}\]

    \[ \homrappXtoYatZ {\homrappXtoYatZ {C_t \alpha} f \beta} {(\homrappXtoYatZ {\homrappXtoYatZ {C_t \beta} g \gamma} h \delta)} \delta
    =
    \homrappXtoYatZ {\homrappXtoYatZ {C_t \alpha} {(\homrappXtoYatZ {\homrappXtoYatZ {C_t \alpha} f \beta} g \gamma)} \gamma} h \delta \]
  \end{enumerate}
\end{definition}
...
\begin{definition}[Internal Functor]
  ...
  \item a map of arrows
    $F_1 : \forall \alpha: {C_0}.
    \homrallXYtoZ \beta {C_0 \alpha \beta} {D_0 {(F_1 \alpha)}{(F_1 \beta)}}$
  \item That preserves identity. This is given by equality between the
    ...
    \[ D_r(F_0\alpha) = \homrappXtoYatZ {F_1\alpha} {C_r \alpha} \alpha\]
  \item And preserves composition.
    This is given by an equality between the following terms of type

    \[ \forall \alpha.
    \homrallXYtoZ {\beta} {C_1 \alpha \beta} {\homrallXYtoZ \gamma {C_1 \beta \gamma} {D_1 {(F_0 \alpha)} {(F_0 \gamma)}}}\]

    First, we can compose in $C$, and then map using $F$:
    \[ \homrappXtoYatZ {F_1 \alpha} {(\homrappXtoYatZ {\homrappXtoYatZ {C_t \alpha} f \beta} g \gamma)}\gamma\]

    
    or we can map the morphisms with $F$, and then compose them in $D$:
    \[ \homrappXtoYatZ {\homrappXtoYatZ {D_t {(F_0 \alpha)}} {(\homrappXtoYatZ {F_1 \alpha} f \beta)} {(F_0 \beta)}} {(\homrappXtoYatZ {F_1 \beta} g \gamma)} {F_0 \gamma}\]
  \end{enumerate}
\end{definition}
