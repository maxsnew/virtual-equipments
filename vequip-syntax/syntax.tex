...
and $R : D^o \times E \to Set$, the cotensor product is given as
...
\begin{figure}
  \begin{mathpar}
      ...
      \inferrule*[right=Contra Cotensor elimination]
...
Cotensor judgments are useful not just in applications (Kan
extensions, the Yoneda lemma), but also for purely syntactic
reasons. In particular, I don't see how to give a sensible, complete
natural deduction-style syntax for the other multiplicative
connectives (Unit (Hom) and Tensor) without relying on the presence of
cotensor judgments. The key property is that they give us a way to
syntactically write a substitution $M[\phi]$ uniformly in $M$, as
``$M$ applied to $\phi$''. In fact it gives us many ways since we can
use a mix of left and right cotensors.

As an example, consider a term $M$ with a single judgment variable
$\alpha : \cat C, x : R, \beta : \cat D \vdash M : S$. Then a
substitution for this term is given by:
\[ A/\alpha, N/x, B/\beta : \Psi \to \alpha : \cat C, x : R, \beta : \cat D \]
Substitution $M[\phi]$ is syntactically defined by induction on $M$,
but using cotensors, we can construct an equivalent term that is
uniform in $M$ by destructuring $\phi$ instead. Specifically, we can
abstract over the variable $x$ on the left or right, and then supply
the input term $N$ instead as an \emph{argument}, rather than
substituting it directly in $M$:
\[ (\lambda^\triangleleft \alpha, x. M[B/\beta]) \triangleleft (A,N) \]
The only issue is we still don't have a way to get at that final
$\beta\ldots$.
...
\begin{figure}
  \begin{mathpar}
    ...
    {\Phi_1[\delta/\delta_1], \delta:\cat C, \Phi_2[\delta/\delta_2]\vdash N : Q[\delta/\delta_1,\delta/\delta_2]}
    {\Phi_1, \delta_1 : \cat C, f : \delta_1 \to_{\cat C} \delta_2, \delta_2 : \cat C, \Phi_2\vdash J(f, \delta. N) : Q}


    \inferrule*[right=Hom-Left-subst]
    {\phi : \Psi \to \Phi_1, \delta_1 : \cat C, f : \delta_1 \to_{\cat C} \delta_2, \delta_2 : \cat C, \Phi_2\and
     \Phi_1[\delta/\delta_1], \delta:\cat C, \Phi_2[\delta/\delta_2]\vdash N : Q[\delta/\delta_1,\delta/\delta_2]
    }
    ...
    %% \inferrule*[right=Hom-Elim (Nat. Deduction)]
    %% {\beta : \cat B \vdash M : D_1 \to_{\cat C} D_2\and
    %%   \Phi[\delta/\delta_1],\delta:\cat D,\Phi'[\delta/\delta_2] \vdash N : R[\delta/\delta_1,\delta/\delta_2]
    %% }
    %% {\Phi,\beta,\Psi \vdash J(M,\beta. N) : R}
    ...
    \inferrule
    {\Phi[\delta/\delta_1],\delta:\cat D, \Psi[\delta/\delta_2] \vdash M[\delta/\delta_1,\id(\delta)/f,\delta/\delta_2] = N[\delta/\delta_1,\id(\delta)/f,\delta/\delta_2] : Q[\delta/\delta_1,\delta/\delta_2]}
