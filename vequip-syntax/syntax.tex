...
\newcommand{\Sort}{\,\,\textrm{Sort}}
\newcommand{\dtctx}{\,\,\textrm{type context}}
\newcommand{\vectx}{\,\,\textrm{cat context}}
...
\begin{figure}
  \begin{mathpar}
    ...
    \inferrule
    {\Gamma \dtctx \and \Gamma \vdash \Xi \boundary}
    {\Gamma \pipe \Xi \vdash S \Set}

    \inferrule{\Gamma \dtctx}{\Gamma \vdash \cat C \sort}
    ...
    {\Gamma \vdash \Xi \boundary}
    {\Gamma \pipe \Xi \vdash \Phi \vectx}

    \inferrule*[right=Term formation]
    {\Gamma \pipe \Xi \vdash \Phi \vectx
     \Gamma \pipe \Xi \vdash R \Set}
    {\Gamma \pipe \Xi \pipe \Phi \vdash t : R}

    \inferrule
    {\Delta \dtctx \and \Gamma \dtctx}
    {\gamma : \Delta \to \Gamma}
    
    \inferrule*[right=Boundary Map formation]
    {\Delta \vdash Z \boundary \and \Gamma \vdash \Xi \boundary
    \and
    \gamma : \Delta \to \Gamma}
    {\gamma,\xi : \Delta \pipe Z \to \Gamma \pipe \Xi}

    \inferrule
        {\gamma,\xi : \Delta \pipe Z \to \Gamma\pipe \Xi\and
          \Delta \pipe Z \vdash \Psi \vectx \and
          \Gamma \pipe \Xi \vdash \Phi \vectx
        }
        {\gamma,\xi,\phi : \Delta\pipe Z \pipe \Psi \to \Gamma\pipe\Xi\pipe \Phi}
  \end{mathpar}
...
\begin{figure}
  \begin{mathpar}
    \inferrule{}{\cdot \dtctx}

    \inferrule{}{\Gamma \vdash \cdot \boundary}
    
    \inferrule
    {\Gamma \dtctx \and \Gamma\pipe \cdot \vdash A \Set}
    {\Gamma , x : A \dtctx}
    
  \end{mathpar}
  \caption{Basic Rules Part 1}
\end{figure}

\begin{figure}
  \begin{mathpar}
    \inferrule
    {\Gamma \vdash \cat C \sort}
    {\Gamma \pipe \alpha:\cat C \vdash \alpha : \cat C}

    \inferrule{\Gamma \vdash \cat C \sort}{\Gamma \vdash \alpha:\cat C \boundary}
    ...
    {\cat C \sort}
    {\alpha : \cat C \vectx(\alpha^\pm : \cat C)}

    \inferrule*[right=CtxExt]
    {\Phi \vectx(\Xi)\and R \prof(d^+\Xi : \cat C; \beta^+ : \cat D)}
    {\Phi,x : R, \beta : \cat D \vectx(d^-(\Xi) ; \beta^+ : \cat D)}

    \inferrule*[right=CtxTensor*]
    {\Phi\vectx(\Xi) \and \Psi \vectx(Z) \and d^+ \Xi = d^- Z}
    {\Phi, \Psi\vectx(\Xi,Z)}\\\\
...
\begin{figure}
  \begin{mathpar}
      ...
      {\Phi \vectx(d^+\Xi; \gamma : \cat E) \and P \prof(d^-\Xi; \gamma : \cat E)}
      ...
      %% {\Phi \vectx(\alpha:\cat C; d^-\Xi) \and P \prof(\alpha : \cat C; d^+\Xi)}
...
\begin{figure}
  \begin{mathpar}
  \inferrule
  {\Gamma \pipe\cdot \vdash S \Set\and \Gamma \pipe\cdot \vdash T \Set}
  {\Gamma\pipe\cdot \vdash \prod_{x:S} T \Set}
  ...
  \inferrule
  {\Gamma \pipe\cdot \vdash S \Set\and \Gamma \pipe\cdot \vdash T \Set}
  {\Gamma\pipe\cdot \vdash \sum_{x:S} T \Set}
  
  \inferrule
  {~}
  {\Gamma\pipe\cdot \vdash \top \Set}
  \end{mathpar}
...
\begin{figure}
  \begin{mathpar}
    \inferrule{\Gamma \dtctx}{\Gamma\pipe\cdot \vdash \Sort \Set}
    
    \inferrule
    {\Gamma \vdash \cat C \sort}
    {\Gamma \vdash \quoth {\cat C} : \Sort}

    \inferrule
    {\Gamma \vdash C : \Sort}
    {\Gamma \vdash \unquoth C \sort}

    TODO: equations\\

    \inferrule
    {\Gamma \vdash \cat C \sort \and\Gamma \vdash \cat D \sort}
    {\Gamma\pipe\cdot \vdash \varr{\cat C}{\cat D} \Set}
    ...
    \inferrule
    {\Gamma \vdash \cat C \sort \and\Gamma \vdash \cat D \sort}
    {\Gamma\pipe\cdot \vdash \harr{\cat C}{\cat D} \Set}

    \inferrule
    {\Gamma \pipe \alpha:\cat C;\beta:\cat D \vdash R \,\Set}
    ...
    {\Gamma \pipe \Xi \vdash M A; B \Set}

    TODO: equations\\

    \inferrule
    {\Gamma \pipe \alpha:\cat C. R \Set}
    {\Gamma \pipe \cdot \vdash \forall \alpha:\cat C. R \Set}
    ...
    \inferrule
    {\Gamma \pipe \cdot \vdash M : \forall \alpha:\cat C. R\and
      \Gamma \pipe \beta:\cat D \vdash A : \cat C}
    {\Gamma \pipe \beta:\cat D \vdash M A : R[A/\alpha]}
  \end{mathpar}
...
\begin{definition}[Internal Category]
  ...
  \item A span $C_1 : \harr{\unquoth C_0}{\unquoth C_0}$
  \item Identity arrows
    $C_r : \forall \alpha : \unquoth C. H \alpha \alpha$
  \item Composition, which we can express several different ways using
    a mix of covariant/contravariant homs and tensors.
    \[ C_t^\triangleright : \forall \alpha : \unquoth C. \forall \beta. (H \beta \alpha) \triangleright \forall \gamma. (H \gamma \beta) \triangleright H \gamma \alpha \]
...
\begin{definition}[Internal Functor]
  ...
  \item a function on arrows
    $F_1 : \forall \alpha:\unquoth {C_0}. \forall \beta:\unquoth{C_0}. D_1 {(F_0 \alpha)}{(F_0 \beta)} \triangleleft C_1 \alpha \beta$
  \item That preserves identity. This is given by equality between the
    following terms of type $\forall \alpha:{\unquoth {C_0}}. D_1 {F_0
      \alpha}{F_0\alpha}$:
